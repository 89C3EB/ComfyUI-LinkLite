// ComfyUI-LinkLite — 轻量连线美化插件 V1（适配 ComfyUI 0.3.x：Map _links / setDirty / 空闲推帧）
...
const SPEED_MAX = 3;                   // 速度滑杆鼠标拖拽的上限
const SPEED_HARD_MAX = 12;             // 隐藏上限：在速度滑杆上按住鼠标中键滚动滚轮可继续加速到此值
const DEFAULTS = {
  enabled: true,
  style: "curve",
  fx: "spider",
...
function drawStyled(canvas, ctx, a, b, link, c, scale, dirs) {
  ...
  try {
    if (typeof window !== "undefined") {
      const dbg = window.__LLDBG || (window.__LLDBG = { styled: 0, last: 0, dt: [], push: 0 });
      const t = performance.now();
      if (dbg.last) { dbg.dt.push(Math.round(t - dbg.last)); if (dbg.dt.length > 40) dbg.dt.shift(); }
      dbg.last = t; dbg.styled++;
    }
  } catch (e) {}
  ...
  // 2) 动效：直接绘制在主画布内（经验证能正常显示）。相位用绝对时间(performance.now)，
  // 每帧都画在正确位置、不顺移。空闲由下方 ensureAnim 的 rAF 恒定推帧
  // (markDirtyCanvas：0.3.x 用 setDirty，旧版回退 setDirtyCanvas)，交互时由 ComfyUI 事件循环驱动。
  if (active && c.fx && c.fx !== "none") {
    ensureFxGeo(geo);
    const fpts = geo.fpts, mm = geo.mm;
    ctx.setLineDash([]);
    switch (c.fx) {
...
function findLinkById(graph, id) {
  ...
  const src = (graph._links != null) ? graph._links : graph.links;
  if (src) {
    let found = null;
    if (typeof src.forEach === "function") {
      src.forEach(function (l) { if (!found && l && (l.id === id || l.id === Number(id))) found = l; });
    } else if (Array.isArray(src)) {
      for (let i = 0; i < src.length; i++) {
        const l = src[i];
        if (l && (l.id === id || l.id === Number(id))) { found = l; break; }
      }
    } else {
      for (const k in src) {
        const l = src[k];
...
// 最近一次真正绘制连线的画布实例：0.3.x 中 app.canvas 可能指向过期实例(graph 为空)，
// 用它推帧等于白推；以 renderLink 实际调用的画布为准。
let g_liveCanvas = null;
...
// 0.3.x 两处不兼容：graph._links 由普通对象改为 Map（Object.keys/for...in 均失效）；
// 置脏方法由 setDirtyCanvas 改名 setDirty。二者叠加导致旧推帧循环静默失效 → 空闲时
// drawBackCanvas 不再执行、特效冻结，只有移动画布(拖拽置脏)才恢复。
function linkCount(g) {
  if (!g) return 0;
  // 0.3.7 实测：主 graph 的 _links 为 undefined，连线在公开 links 属性；
  // 旧版 litegraph 在 _links。两者都可能是 Map/数组/普通对象。
  const src = (g._links != null) ? g._links : g.links;
  if (!src) return 0;
  if (typeof src.size === "number") return src.size;
  if (Array.isArray(src)) return src.length;
  return Object.keys(src).length;
}
function markDirtyCanvas(c) {
  if (!c) return;
  try {
    if (typeof c.setDirty === "function") c.setDirty(true, true);
    else if (typeof c.setDirtyCanvas === "function") c.setDirtyCanvas(true, true);
    else { c.dirty_bgcanvas = true; c.dirty_canvas = true; }
  } catch (e) {}
}
function ensureAnim() {
  if (rafStarted) return;
  rafStarted = true;
  // 空闲恒定推帧：markDirtyCanvas 置脏前台+后台，让主画布内的特效持续流动
  // (交互时由 ComfyUI 事件循环驱动，同频)。相位用绝对时间，每帧画在正确位置，
  // 不因帧率波动顺移/跳变。仅在有连线时推帧，空图零开销。
  // 诊断：浏览器控制台输入 window.__LLDiag 可查看推帧状态。
  let lastPush = 0;
  const diag = (typeof window !== "undefined")
    ? (window.__LLDiag = window.__LLDiag || { frames: 0, pushes: 0, links: -1, graph: "" })
    : null;
  const tick = (now) => {
    requestAnimationFrame(tick);
    if (typeof document !== "undefined" && document.hidden) return;
    let c = null;
    try { c = (g_liveCanvas && g_liveCanvas.graph) ? g_liveCanvas : app.canvas; }
    catch (e) { try { c = app.canvas; } catch (e2) { return; } }
    // app.canvas 过期(graph 无连线)时，回退到 litegraph 全局活跃画布
    try {
      if (c && c.graph && linkCount(c.graph) === 0) {
        const AC = globalThis.LiteGraph && globalThis.LiteGraph.LGraphCanvas;
        const ac = AC && AC.active_canvas;
        if (ac && ac !== c && ac.graph && linkCount(ac.graph) > 0) c = ac;
      }
    } catch (e) {}
    if (!c || !cfg.enabled || !cfg.fx || cfg.fx === "none" ||
        !now || now - lastPush < 16) return;
    let g = null;
    try { g = c.graph; } catch (e) { return; }
    if (diag) {
      diag.frames++;
      if (g) {
        diag.links = linkCount(g);
        diag.graph = (g.constructor && g.constructor.name) || typeof g;
      }
    }
    if (!g || linkCount(g) === 0) return;
    lastPush = now;
    if (diag) diag.pushes++;
    markDirtyCanvas(c);
    // ComfyUI 空闲节能可能停掉渲染循环；置脏无人消费时重启它
    try {
      if (!c.is_rendering && typeof c.startRendering === "function") c.startRendering();
    } catch (e) {}
  };
  requestAnimationFrame(tick);
}
...
function renderPanel() {
  ...
  $("#lt-reset").onclick = () => { cfg = Object.assign({}, DEFAULTS); lang = resolveLang(); save(); renderPanel(); markDirtyCanvas(app.canvas); };
...
function showButton() { ensureButton(); btn.title = tr().title + "  V1"; }
...
app.registerExtension({
  ...
  async setup() {
    ...
    if (patched) {
      console.info("%c[LinkLite V1] %c已加载 / loaded (%c0.3.x 空闲推帧已修复：setDirty + Map _links)",
        "color:#7aa6ff", "color:#888", "color:#7cf0a0");
    } else {
      console.warn("[LinkLite V1] 未能接入渲染管线（renderLink / renderLinkDirect / drawLink 均不存在），本次未启用（不影响其他功能）");
    }
    ensureAnim();
    showButton();
    showBadge();
